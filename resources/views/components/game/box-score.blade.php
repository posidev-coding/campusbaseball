<div class="flex justify-center">
    ...
    <div @class([
        'border dark:border-slate-500 rounded-lg mt-2 md:mt-4 lg:mx-4 grow',
        ...
        <div class="flex items-stretch w-full">
            <div class="flex flex-col">
                <div class="bg-slate-100 dark:bg-slate-700 h-7 border-b dark:border-b-slate-500 rounded-tl-lg"></div>
                <div class="flex items-center space-x-2 px-2 h-7 border-b dark:border-b-slate-500 dark:bg-slate-800">
                    @isset($this->game->away->logo)
                        <x-game.team-logo :team="$game->away" size="5" />
                    @endisset
                    <div @class([
                        'text-sm font-light',
                        'text-black dark:text-zinc-100 font-medium' => $this->game->away_winner,
                        'text-gray-400 dark:text-zinc-300' => !$this->game->away_winner,
                    ])>{{ $this->game->away->abbreviation ?? 'N/A' }}</div>
                </div>
                <div class="flex items-center space-x-2 px-2 h-7 dark:bg-slate-800 rounded-bl-lg">
                    @isset($this->game->home->logo)
                        <x-game.team-logo :team="$this->game->home" size="5" />
                    @endisset
                    <div @class([
                        'text-sm font-light',
                        'text-black dark:text-zinc-100 font-medium' => $this->game->home_winner,
                        'text-gray-400 dark:text-zinc-300' => !$this->game->home_winner,
            ...
            <div class="flex grow">
                ...
                @foreach($innings as $inning)
                    ...
                    <div class="flex flex-col grow border-l dark:border-l-slate-500 text-sm font-light">
                        <div @class([
                            'h-7 place-content-center text-center text-gray-500 dark:text-white bg-slate-100 dark:bg-slate-700 border-b dark:border-b-slate-500',
                            'text-gray-900 font-semibold' =>!$this->game->final && $inning['away']['inning'] == $this->game->status['period'],
                        ])>{{ $inning['away']['inning'] }}</div>
                        <div @class([
                            'h-7 place-content-center text-center text-gray-700 dark:text-zinc-300 border-b dark:border-b-slate-500 dark:bg-slate-800',
                            'bg-blue-50' =>
                                !$this->game->final && $inning['away']['inning'] == $this->game->status['period'] && $this->game->status['periodPrefix'] == 'Top',
                        ])>{{ $inning['away']['runs'] }}</div>
                        <div @class([
                            'h-7 place-content-center text-center text-gray-700 dark:text-zinc-300 dark:bg-slate-800',
                            'bg-blue-50' =>
                ...
                <div class="flex flex-col grow border-l dark:border-l-slate-500 font-semibold dark:font-medium text-sm">
                    <div class="h-7 place-content-center text-center text-gray-500 dark:text-white bg-slate-100 dark:bg-slate-700 border-b dark:border-b-slate-500">R</div>
                    <div class="h-7 place-content-center text-center text-gray-700 dark:text-gray-200 border-b dark:border-b-slate-500 dark:bg-slate-950">
                        {{ $this->game->away_runs ?? '-' }}
                    </div>
                    <div class="h-7 place-content-center text-center text-gray-700 dark:text-white dark:bg-slate-950">
                        {{ $this->game->home_runs ?? '-' }}
                    </div>
                </div>
                <div class="flex flex-col grow border-l dark:border-l-slate-500 font-semibold dark:font-medium text-sm">
                    <div class="h-7 place-content-center text-center text-gray-500 dark:text-white bg-slate-100 dark:bg-slate-700 border-b dark:border-b-slate-500">H</div>
                    <div class="h-7 place-content-center text-center text-gray-700 dark:text-white border-b dark:border-b-slate-500 dark:bg-slate-950">
                        {{ $this->game->away_hits ?? '-' }}
                    </div>
                    <div class="h-7 place-content-center text-center text-gray-700 dark:text-white dark:bg-slate-950">
                        {{ $this->game->home_hits ?? '-' }}
                ...
                <div class="flex flex-col grow border-l dark:border-l-slate-500 font-semibold dark:font-medium text-sm">
                    <div class="h-7 place-content-center text-center text-gray-500 dark:text-white bg-slate-100 dark:bg-slate-700 border-b dark:border-b-slate-500 rounded-tr-lg">E</div>
                    <div class="h-7 place-content-center text-center text-gray-700 dark:text-white border-b dark:border-b-slate-500 dark:bg-slate-950">
                        {{ $this->game->away_errors ?? '-' }}
                    </div>
                    <div class="h-7 place-content-center text-center text-gray-700 dark:text-white dark:bg-slate-950 rounded-br-lg">
