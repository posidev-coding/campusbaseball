<div wire:poll.30s="refresh">
    ...
    <div class="flex justify-center">
        <div class="flex grow items-center lg:mx-4 lg:py-2">
            ...
            <div class="flex items-center grow justify-end text-lg text-gray-900 font-bold">
                ...
                <div class="flex flex-col items-end min-w-12 pl-4">
                    ...
                    <div class="flex items-center">
                        @if ($game->away_rank > 0)
                            <span
                                class="text-[11px] sm:text-sm lg:font-medium text-gray-500 dark:text-slate-300 font-normal mr-1.5">{{ $game->away_rank }}</span>
                        @endif
                        @if ($game->away_id > 0)
                            <a href="{{ route('team', $game->away_id) }}"
                                class="hover:text-blue-800 dark:text-slate-300 dark:hover:text-white">
                                <span
                                    class="hidden md:flex font-normal lg:font-semibold lg:tracking-wide">{{ $game->away->location }}</span>
                                <span
                                    class="flex md:hidden text-[11px] sm:text-sm font-normal">{{ $game->away->abbreviation }}</span>
                            </a>
                        @else
                    ...
                    @if (isset($game->away_records))
                        <div
                            class="flex items-center font-light md:font-normal text-[10px] sm:text-[11px] md:text-xs space-x-1 text-gray-500 dark:text-slate-300 p-0">
            ...
            <!-- Status -->
            <div
                class="flex flex-col shrink items-center space-y-0.5 px-2 md:px-4 lg:px-6 text-sm font-light text-muted dark:text-lighter">
            ...
            <div class="flex grow items-center justify-start text-lg text-gray-900 font-bold">
                ...
                <div class="flex flex-col items-start pr-4 min-w-12">
                    ...
                    <div class="flex items-center">
                        @if ($game->home_rank > 0)
                            <span
                                class="text-[11px] sm:text-sm lg:font-medium text-gray-500 dark:text-slate-300 font-normal mr-1.5">{{ $game->home_rank }}</span>
                        @endif
                        @if ($game->home_id > 0)
                            <a href="{{ route('team', $game->home_id) }}"
                                class="hover:text-blue-800 dark:text-slate-300 dark:hover:text-white">
                                <span
                                    class="hidden md:flex font-normal lg:font-semibold lg:tracking-wide">{{ $game->home->location }}</span>
                                <span
                                    class="flex md:hidden text-[11px] sm:text-sm font-normal">{{ $game->home->abbreviation }}</span>
                            </a>
                        @else
                            <span class="text-gray-500 dark:text-slate-300">TBD</span>
                        @endif
